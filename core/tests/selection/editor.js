 ...
 *
 * @title: Core: selection - tests for editor instance
 * @tags: editor, unit
 */
...
CKTESTER.test( {
	assertGetSelection : function( source, expected ) {
		var ed = this.editor;
		CKTESTER.tools.setHtmlWithSelection( ed, source );
		assert.areSame( expected || source, CKTESTER.tools.getHtmlWithSelection( ed ) );
	},
	...
	'test selection on initial focus': function() {
		var ed = this.editor;
		ed.editable().focus();
		assert.areEqual( '<p>^foo</p>', CKTESTER.tools.getHtmlWithSelection( ed ), 'Selection goes into editable on focus (#9507).' );
	},
	...
	'test selection on initial focus - ensure new doc': function() {
		...
		setTimeout( function() {
			ed.setData( '<p>foo</p>', function() {
				resume( function() {
					ed.editable().focus();
					assert.areEqual( '<p>^foo</p>', CKTESTER.tools.getHtmlWithSelection( ed ), 'Selection goes into editable on focus (#10115).' );
				} );
			} );
		} );
		wait();
	...
	'test collapsed text selection' : function() {
		...
		if ( !CKEDITOR.env.ie ) {
			...
			// Opera produces an extra BR with <hr /> at the start of content.
	...
	'test selection after DOM unload' : function() {
		...
		editor.focus();
		CKTESTER.tools.setHtmlWithSelection( editor, '<p>foo^bar</p>' );
			...
			setTimeout( function() {
				editor.setData( '', function() {
					resume( function() {
						...
						editable.focus();
						assert.isMatching( /<p>\^.*<\/p>/, CKTESTER.tools.getHtmlWithSelection( editor ), 'Selection is in the right place.' );

						// On Opera focus is async, so ignore it.
						// On the rest check if DOM was fixed before focus is fired.
						if ( !CKEDITOR.env.opera )
