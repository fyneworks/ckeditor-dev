			...
			var widgetDefinition = {
				...
				data: function() {
					...
					CKEDITOR._.oembedCallbacks.push( function( result ) {
						...
						that.data.providerName = result.provider_name;

						// Youtube adjustment to insert flash wmode.
		...
		'default': {
			...
			upcast: function( element, data ) {
				...
				if ( children.length && ( children[ 0 ].type != CKEDITOR.NODE_TEXT || CKEDITOR.tools.trim( children[ 0 ].value ) ) ) {
					...
					if ( element.attributes[ 'data-oembed-provider' ] )
						data.providerName = element.attributes[ 'data-oembed-provider' ];

					if ( data.providerName == 'YouTube' ) {
						element.getFirst().getFirst().attributes.src += '&wmode=transparent';
					}
			...
			downcast: function() {
				...
				ret.attributes[ 'data-oembed-provider' ] = this.data.providerName;

				if ( this.data.providerName == 'YouTube' ) {
					var iframeAttribs = ret.getFirst().getFirst().attributes;
					iframeAttribs.src = iframeAttribs.src.replace( /&amp;wmode=transparent/g, '' );
				}
